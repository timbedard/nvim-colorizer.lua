...
local function color_name_parser(line, i)
  ...
  if prefix then
    ...
    if
      #line >= next_byte_index and byte_is_alphanumeric(line:byte(next_byte_index))
    then
      return
    end
...
do
  ...
  function css_fn.rgba(line, i)
    ...
    local rgb_hex = tohex(
      bor(lshift(floor(r * a), 16), lshift(floor(g * a), 8), floor(b * a)),
      6
    )
    return match_end - 1, rgb_hex
  end
...
local function highlight_buffer(buf, ns, lines, line_start, options)
  ...
  local data = {}
  local mode = options.mode == "background" and { mode = "background" }
    or { mode = "foreground" }
